    ...
    public class ShootingRange : IDataAccessObject
    {
        ...
        public async Task<int> SaveAsync()
        {
            if (ID == -1)
            {
                ...
                await Shell.Current.DisplayAlert("Debug", "Saving sub range" + ID, "okay");
                return ID;
            }
        ...
        {
            DataCacherService<SubRange> service = ServicesHelper.GetService<DataCacherService<SubRange>>();
            foreach (var item in service.GetAllBy(n => n.ShootingRange_ID == ID))
            {
        ...
        {
            SubRange subRange = new SubRange(ID, 0, 0, 0, 0, string.Empty);
            await ServicesHelper.GetService<DataCacherService<SubRange>>().AddOrUpdateAsync(subRange);
        }
